...
private $GETCARBYID="SELECT * FROM vozila WHERE idvozila=?";

private $UPDATECAR="UPDATE vozila SET imeproizvodjaca=?, model=?, kategorija=?, godiste=?,cena=? WHERE idvozila=?";

private $LOGIN ="SELECT * FROM korisnici WHERE username = ? AND password = ?";

private $INSERTUSER="INSERT INTO korisnici(ime,prezime,email,username,password) VALUES(?,?,?,?,?)";
    ...
    public function getDriverById($idvozaca)
    {
        ...
        $statement->bindValue(1, $idvozaca);
        $statement->execute();
        $result = $statement->fetch(); //hvata jedan red
        return $result;
    }
    ...
    public function getCarById($idvozila)
    {
    ...
    public function updateCar($imeproizvodjaca,$model,$kategorija,$godiste, $cena, $idvozila)
    {
        ...
        $statement->bindValue(4,$godiste);
        $statement->bindValue(5,$cena);
        $statement->bindValue(6,$idvozila);
        $statement->execute();
    ...
    public function insertUser($ime,$prezime ,$email,$username,$password) {
